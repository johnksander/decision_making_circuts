...
%within-pool bistable, I increased the E-E strength here and longer pulses
%work now. Weaker pulses will work at longer durations, but the .2 nA
%current I was using at .05 second duration was basically the minimum
%current required to get spiking. 

%So, in order to try out a longer duration pulse with weaker current, I
%increased the E-E connection strength so weaker currents would generate
...
Pr = .05; %release probability
Td = .4;%synaptic depression time constant, seconds
...
Tsra = 50e-3; %adaptation current time constant, ms
%----timecourse---------------
tmax = 4; %simulation end (s)
timestep = .2e-3; %.01 milisecond timestep
timevec = 0:timestep:tmax;
switch current_pulse
    case 'on'
        ptime = [1,1.3];
        pulse_current = .196e-9; %below like, .3 or something it oscilates 
