...
FIRcurrents = .1e-9:.01e-9:.4e-9;
FIRcurrents = [FIRcurrents,.4e-9 - [.01e-9:.01e-9:.3e-9]];
...
for trialidx = 1:num_trials
    ...
    if trialidx == 1
        ...
        Iapp = addpulse(pulse_params,Iapp,timevec); %add pulses
        %these cary over
        V(:,1) = V(:,end); %inital value of membrane potential is last from previous trial
        V(:,2:end) = NaN; %just to be safe
        Isra(:,1) = Isra(:,end);
        Isra(:,2:end) = NaN;
        Sg(:,1) = Sg(:,end);
        Sg(:,2:end) = NaN;
        D(:,1) = D(:,end); %initalize at one
        D(:,2:end) = NaN;
    end
...
Xspot = 1:numel(FIRcurrents);
plot(Xspot,Erates,'linewidth',2)
hold on
plot(Xspot,Irates,'linewidth',2)
hold off
title({'FIR curve, values not reset','Only one pool with E-E connections'})
ylabel('average cell spike Rate (Hz)')
Xcurrents = FIRcurrents / 1e-9;
Xcurrents = Xcurrents(1:10:end);
set(gca,'XTick',Xspot(1:10:end),'XTickLabel', arrayfun(@num2str, Xcurrents(:), 'UniformOutput', false))
xlabel('Iapp (nA)')
legend('Excitatory cells','Inhibitory cells')
print(fullfile(basedir,'switching_simv8_FIRcurve_varycurrent'),'-djpeg')
