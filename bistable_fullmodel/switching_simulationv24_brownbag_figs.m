...
fig_dir = fullfile(basedir,'Results/brownbag');

%----parameterization from v23_testPS
%This is intended to eye-ball test networks from the parameter sweep

force_back2stay = false;
options.Rstim = 0; %rate for stimulus input spikes

options.EtoE  = (.0675 * .6); %from 21d
options.ItoE  = (4.15 * .15);  
options.EtoI = .175; 

% options.EtoE = .0405; %from parsweep
% options.ItoE = 1.288;
% options.EtoI = .1734;
...
raster = spikeplot(celltype.excit,:);
imagesc(raster)

Xticks = num2cell(get(gca,'Xtick'));
Xticks = Xticks(cellfun(@(x) mod(x,1/timestep) == 0,Xticks)); %only seconds
Xticks = [{1},Xticks];
...
ylabel({'excitatory cells';' '},'FontWeight','b')
...
figure(5)
subplot(2,1,1)
plot(V(example_cells(1),:),'LineWidth',1.5,'Color',matorange)
axis tight
ylim = get(gca,'YLim');
ylim(2) = 0;
set(gca,'YLim',ylim);
title('Stay pool excitatory cell','FontWeight','b')
%xlabel('time (s)','FontWeight','b')
ylabel({'membrane potential', '(mV)'},'FontWeight','b')
Xticks = num2cell(get(gca,'Xtick'));
Xticks = Xticks(cellfun(@(x) mod(x,1/timestep) == 0,Xticks)); %only seconds
Xticks = [{1},Xticks];
Xlabs = cellfun(@(x) sprintf('%i',round(x*timestep)),Xticks,'UniformOutput', false); %this is for normal stuff
...
subplot(2,1,2)
plot(V(example_cells(1) + sum(celltype.pool_stay),:),'LineWidth',1.5,'Color',matorange)
axis tight
ylim = get(gca,'YLim');
ylim(2) = 0;
set(gca,'YLim',ylim);
title('Leave pool excitatory cell','FontWeight','b')
xlabel('time (s)','FontWeight','b')
ylabel({'membrane potential', '(mV)'},'FontWeight','b')
Xticks = num2cell(get(gca,'Xtick'));
Xticks = Xticks(cellfun(@(x) mod(x,1/timestep) == 0,Xticks));
Xticks = [{1},Xticks];
Xlabs = cellfun(@(x) sprintf('%i',round(x*timestep)),Xticks,'UniformOutput', false); %this is for normal stuff
set(gca,'Xdir','normal','Xtick',cell2mat(Xticks),'XTickLabel', Xlabs);
set(gca, 'TickLength',[0 0])
set(gca,'FontSize',fontsz)
print(fullfile(fig_dir,'example_cells'),'-djpeg')
%hold off
