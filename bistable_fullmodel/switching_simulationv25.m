...
for idx = 2:numel(timevec)
    ...
    if idx > numel(timevec)*.25 && idx < numel(timevec)*.75 
         % do half-noise for "undecided state"
         ext_spikes = poissrnd(Lext*.5,pool_options.num_cells,1); %external spikes to noise conductance
    else %regular noise 
