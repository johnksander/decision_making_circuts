...
%tryna redo this & make it better
%parameterization changed from v21. This gives less realistic spikerates,
%but the network satisfies:
%   a) fast switching when all things are equal 
%   b) stimulus prolongs switching 
%very well.
%yea, this works much better
...
W(W > 0 & EtoE) = (.0675 * .6);   
W(W > 0 & ItoE) = (4.15 * .15);  %increase
W(W > 0 & EtoI) = .175; %decrease
...
Rstim = 207.5; %rate for stimulus input spikes 
%210 might be too long
%205 might be best bet...
%250 gives okay switching, %275 gives like no switching at all 
%Rstim = 0; %rate for stimulus input spikes 
stim_targs = celltype.excit & celltype.pool_stay;
...
%Lstim = Rstim * timestep; %lambda for stimulus  
...
%stim_info.stimB_lambda = 0 * timestep;
...
for idx = 2:numel(timevec)
    ...
    %stim_spikes(stim_targs) = poissrnd(Lstim,sum(stim_targs),1);
