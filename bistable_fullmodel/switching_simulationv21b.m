...
%same as v21, adding the stimulus which should slow switching. Shouldn't be
%hard... 
%Result: this gives more realisitic spikerates, but the stimulus doesn't
%prolong the switching very well. 
...
% W(W > 0 & EtoE) = .0675 * .8;   %.0675 * 1
% W(W > 0 & ItoE) = (4.15 * .15);    %(4.15 * .15)
% W(W > 0 & EtoI) = .175;    %.26
W(W > 0 & EtoE) = .0675;   %.0675 * 1
W(W > 0 & ItoE) = (4.15 * .15);    %(4.15 * .15)
W(W > 0 & EtoI) = .26;    %.26
...
Rext = 1500; %poisson spike train rate for noise, Hz
Rstim = 500; %rate for stimulus input spikes 
