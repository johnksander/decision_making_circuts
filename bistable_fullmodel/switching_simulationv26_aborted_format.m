...
%12/13/18: this was mostly adapted to new spikeout model functionality. 
%it wasn't any faster than the diagmodel so might as well just port that
%code directly instead of trying to pick up bugs 
...
%---last init-----------------
experiment_set2go = true; %true skips bistability check 
...
state.now = state.undecided; %this is always true if V initialized at El
...
for idx = 2:numel(timevec)
    ...
    if ~avail_stim
        %do not do this for current testing
    ...
    %progress tracking...
    if mod(idx,floor(num_timepoints * .10)) == 0 %10 percent
        progress = (idx /  num_timepoints) * 100;
