...
config_options.sim_name = 'NFSv5_Istay';
options = set_options(config_options);
options.current_pulse = 'off';  %'on' | 'off' set off for low memory implementation
options.tmax = 5000; %trial simulation time (s) 
stimA = ones(24,1); %let's do 2 runs per core at 4300k seconds, try and get 10k switches 
...
options.NFS = 'Istay'; %cells driving forced switch:  Estay | Eswitch | Istay | Iswitch
options.NFS_onset_min = .75; %minimum state duration for forced switch (seconds)
options.NFS_recordwindow = [.95 10]; %only record switches in this window (seconds)
options.NFS_stoppush = max(options.NFS_recordwindow)-options.NFS_onset_min; %noise push duration (s)
options.NFS_noisepush = 50e-12; %push noise by 50 picoamps 
