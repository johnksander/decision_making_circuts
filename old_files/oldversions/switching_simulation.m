...
%increase depression
%W(W > 0 & EtoE) = 10; %watch out for how this is selected (W ~=0) if you add noise or something


%set up parameters
%--------------------------------------------------------------------------
%----cell connections---------
Erev_excit = 0; %reversal potential for excitatory synapses
Erev_inhib = -70e-3; %reversal potential for inhibitory synapses
...
for idx = 2:numel(timevec)
    ...
    Isyn = W'*Sg(:,idx-1).*Gg;
    %excitatory
    dVdt = ((El-V(excit,idx-1)+(delta_th.*exp((V(excit,idx-1)-Vth)./delta_th)))./Rm)...
        + (Isyn(inhib).*(Erev_inhib - V(excit,idx-1)))... %Isyn input & reverse potentential from inhib
        - Isra(excit,idx-1) + Iapp(excit,idx-1); %noise is already in Iapp
    V(excit,idx) = ((dVdt./Cm) .* timestep) + V(excit,idx-1);
    %inhibitory
    dVdt = ((El-V(inhib,idx-1)+(delta_th.*exp((V(inhib,idx-1)-Vth)./delta_th)))./Rm)...
        + (Isyn(excit).*(Erev_excit - V(inhib,idx-1)))... %Isyn input & reverse potentential from excit
        - Isra(inhib,idx-1) + Iapp(inhib,idx-1); %noise is already in Iapp
    V(inhib,idx) = ((dVdt./Cm) .* timestep) + V(inhib,idx-1);
