...
num_cells = 200;
pool_options.sz_stay = .5; %proportion stay out of total 
pool_options.sz_excitable = .5; %proportion excitable out of stay pool (this is asusming equal excitable n across pools)
...
%increase depression
%W(W > 0 & ItoE) = 1000; %watch out for how this is selected (W ~=0) if you add noise or something
...
Erev(excit) = 0; %reversal potential for excitatory synapses
Erev(inhib) = -70e-3; %reversal potential for inhibitory synapses
...
Tsyn(excit) = 50e-3; %excitatory gating time constant, ms 
...
%     end
    ...
    Isyn = W'*Sg(:,idx-1).*Gg;
    %stay
    dVdt = ((El-V(pool_stay,idx-1)+(delta_th.*exp((V(pool_stay,idx-1)-Vth)./delta_th)))./Rm)...
        + (Isyn(pool_switch).*(Erev(pool_switch) - V(pool_stay,idx-1)))... %Isyn input & reverse potentential from switch pool
        - Isra(pool_stay,idx-1) + Iapp(pool_stay,idx-1); %noise is already in Iapp
    V(pool_stay,idx) = ((dVdt./Cm) .* timestep) + V(pool_stay,idx-1);
    %switch
    dVdt = ((El-V(pool_switch,idx-1)+(delta_th.*exp((V(pool_switch,idx-1)-Vth)./delta_th)))./Rm)...
        + (Isyn(pool_stay).*(Erev(pool_stay) - V(pool_switch,idx-1)))... %Isyn input & reverse potentential from stay pool
        - Isra(pool_switch,idx-1) + Iapp(pool_switch,idx-1); %noise is already in Iapp
    V(pool_switch,idx) = ((dVdt./Cm) .* timestep) + V(pool_switch,idx-1);
