...
%problem is they're "bouncing out" or something right now...
...
W(W > 0 & EtoE) = .025; %watch out for how this is selected (W ~=0) if you add noise or something
%--------------------------------------------------------------------------
...
Gg(celltype.excit) = 1e-6; %excitatory max connductance microSiemens
Gg(celltype.inhib) = 1e-6; %inhibitory max connductance microSiemens
...
a = 1e-9; %adaptation conductance, in nano Siemens
%b = .02e-9;%increase adaptation current by max value, nano amps
b = 2e-9;%increase adaptation current by max value, nano amps 
...
set(gca,'Xdir','normal','XTick', Xtick_pos,'XTickLabel', arrayfun(@num2str, Xtick_seconds(:), 'UniformOutput', false))
title({'spikes','(spikes in matrix enlarged for visualization)'})
ylabel({'excitatory cells:    #1-50','inhibitory cells:    #51-100'})
xlabel('time (s)')
print('oscillation_problem_fig1','-djpeg')
...
set(gca,'Xdir','normal','XTick', Xtick_pos,'XTickLabel', arrayfun(@num2str, Xtick_seconds(:), 'UniformOutput', false))
hold off
print('oscillation_problem_fig2','-djpeg')
