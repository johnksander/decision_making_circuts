...
for idx = 2:numel(timevec)
    ...
    %         Iapp(celltype.pool_stay,idx) = Iapp(celltype.pool_stay,idx) + 5e-9; %zap it
    %     end
    ...
    %                       ((El-V(:,idx-1)+(delta_th.*exp((V(:,idx-1)-Vth)./delta_th)))./Rm) - Isra(:,idx-1) %plus I accounts for ...
    %                                                                                             % + (Isyn.*(Erev - V(:,idx-1))) + Iapp(:,idx-1);
    %      I = I + (E_revE - v(:,j)).*(G(:,1:NE)*S(1:NE,j)); %I includes noise from previous lines
    %I = I + (E_revI - v(:,j)).*(G(:,NE+1:NE+NI)*S(NE+1:NE+NI,j));
    I = Iapp(:,idx-1);
    ...
    %     Isyn = W'*Sg(:,idx-1).*Gg; %don't do this with conductance
    %     %all together now
