...
switch disable_init_pulse
    case 'yes'
        title({'Paul''s model with equal pool currents','spiking matrix reordered by pool (pool A = rows 1:125)'})
        print('pauls_model_EqualCurrents','-djpeg')
    case 'no'
        title({'Paul''s model with equal pool currents & inital pulse','spiking matrix reordered by pool (pool A = rows 1:125)'})
        print('pauls_model_EqualCurrents_and_InitalPulse','-djpeg')
end
