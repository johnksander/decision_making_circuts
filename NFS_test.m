...
config_options.sim_name = 'NFS_Iswitch_test';
options = set_options(config_options);
options.current_pulse = 'off';  %'on' | 'off' set off for low memory implementation
options.tmax = 200; %trial simulation time (s) 
stimA = ones(4,1); %let's do 1 run per core at 10k seconds, try and get 10k switches 
...
options.NFS_noisepush = 100e-12; %push noise by X picoamps 
...
%---run-----------------------
modelfile = NFS_testmodel(options);
%---cleanup-------------------
