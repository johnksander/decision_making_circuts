...
%note-- 4/23/19: check when pool Y activity is (some %) pool X, or when 
%pool Y > pool X. For instance, when is I-stay rate = half the E-stay rate 
...
opt.Ypool = 'I-stay'; 
opt.Xpool = 'I-switch'; 
opt.pool_dif = .5; % decimal between 0-1. If pool_diff = 1, then it's >=
...
for Sidx = 1:numel(Snames)
    ...
    for Tidx = 1:numel(timewins)
        ...
        for Didx = 1:numel(treatments)
            ...
            for Pidx = 1:numel(pool_diff)
                
                opt.pool_dif = pool_diff(Pidx);
...
mainfig_dir = fullfile(home_dir,'Results',figdir,'Twise_difftime');
...
fig_dir = fullfile(mainfig_dir,sprintf('%s_%s',opt.Ypool,opt.Xpool)); %strrep(opt.treat_data,'none','no_treatment'));
data_fn = sprintf('data_%s_%idiff_%s.mat',sim_name,opt.pool_dif*100,opt.Tcourse);
...
fig_fn = sprintf('%s_%idiff_%s',sim_name,opt.pool_dif*100,opt.Tcourse);
...
if ~load_summary
    ...
    parfor idx = 1:num_files
        ...
        for Ti = 1:numel(fdata)
            Y_trace = fdata{Ti}(Ydata_inds,:);
            Y_trace = Y_trace - Y_trace(1); 
            Y_trace(Y_trace < eps) = eps;
            
            X_trace = fdata{Ti}(Xdata_inds,:);
            X_trace(X_trace < eps) = eps;
            
            outcome =  Y_trace ./ X_trace;
            outcome = find(outcome >= opt.pool_dif,1,'first');
            outcome = (outcome - onset_switch)  * timestep;
            fdata{Ti} = outcome;
        end
...
for idx = 1:num_pairs
    ...
    for j = 1:2
        ...
        if plt_idx == 9 || plt_idx == 10
            xlabel(sprintf('T - state transition (ms)\n%s rate > %i%% %s rate',opt.Ypool,opt.pool_dif*100,opt.Xpool))
        end
