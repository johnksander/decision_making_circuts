...
for idx = 1:num_jobs
    ...
    %collapse & reallocate
    result_data{idx,1} = cellcat(file_data(curr_file,1),3);
...
for idx = 1:num_pairs
    ...
    for j = 1:2
        ...
        %result statistics would go here
        curr_data = mean(curr_data,3);
       
        %plot cross correlations
        plot(laginds,curr_data,'Linewidth',lnsz);
        curr_lns = num2cell(get(gca,'Children'));
        cellfun(@(x,y) set(x,'Color',y),curr_lns,lncols(Xdata_inds))  
        ...
        if plt_idx == 9 || plt_idx == 10
            xlabel(sprintf('%s lag offset (ms)',opt.Ypool))
        end
