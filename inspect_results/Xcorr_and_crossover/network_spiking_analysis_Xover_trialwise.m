...
Xgroups = {'E-switch','E-stay'};
Tcourse = 'preswitch'; %'preswitch' | 'all'
treat_data = 'minmax'; % zscore | base0 | minmax
agg_type = 'trialwise'; %'trialwise' | 'cellwise'
%cellwise needs a totally different implementation... 
%there's too much data to keep track of every trial to have all networks loaded at once
%you need to load & analyze networks individually (damnit...) 
Nstraps = 25e3;
print_anything = 'yes';
...
fig_dir = fullfile(fig_dir,agg_type);
switch treat_data
    case 'zscore'
...
switch agg_type
    case 'trialwise'%collapse this way
        pool_inds = cellfun(@(x) celltype.(x), strrep(legend_labels,'-',''),'UniformOutput',false);
        result_data = cell(size(result_data));%reset this so it plays nice with cat()
end
...
for idx = 1:num_files
    ...
    switch agg_type
        case 'trialwise'
            %take the mean spikes from cell pool, store every trial 
            fdata = cellfun(@(x) mean(fdata(x,:,:,:),1),pool_inds,'UniformOutput',false)';
            fdata = cat(1,fdata{:});
            result_data{p} = cat(3,result_data{p},fdata);
        case 'cellwise'
...
switch agg_type
    case 'trialwise'
        save('checkpoint_trialwise','result_data','switch_counts')
        keyboard
    case 'cellwise'
