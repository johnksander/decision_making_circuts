...
for idx = 1:numel(ftypes)
    figure;orient tall
    for fidx = 1:Ns
        sp(fidx) = subplot(Ns,1,fidx);
        fn = fullfile(figdir,[Snames{fidx} '_' ftypes{idx} '.fig']);
        h = openfig(fn);pause(.2)
        ax = gca;
        aux=get(ax,'children');
        ...
        for j=1:size(aux)
            fig(fidx) = aux(j);
            copyobj(fig(fidx),sp(fidx));pause(.2)
            hold on
        end
        hold off
        axes(sp(fidx));axis tight;pause(.2)
        xlabel(xlab);ylabel(ylab);pause(.2)
        legend(sp(fidx),leg,'Location','best','box','off');pause(.2)
        set(gca,'Fontsize',14);pause(.3)
        close(h);pause(.2)
    end
    linkaxes(sp,'xy')
    
    Fdir = fullfile(savedir,ftypes{idx});
    print(fullfile(Fdir,['combined_' ftypes{idx}]),'-djpeg')
    close all
end
...
%     [x.ItoE, x.EtoI,x.trial_stimuli,stimtarg_labels(find(strcmpi(x.stim_targs, stimtarg_vals)))],...
%     file_data(1,2),'UniformOutput',false); %matching "network_pair_info" format
% job_params = job_params{1};
...
curr_net_type = job_params.stim_targs;
curr_net_type = stimtarg_labels{strcmpi(curr_net_type, stimtarg_vals)};
%get the right color
...
% file_data = result_data(net_ind,1);
...
if ~isempty(file_data) %skip plot if no data...
    switch outcome_stat
        case 'logmu'
            file_data = file_data(file_data ~= 0);%inf errors
            file_data = log10(file_data);
    end
    histogram(file_data,'FaceColor',lcol,'EdgeColor',lcol,'FaceAlpha',alph);
    main_in_leg = true;main_mu = mean(file_data);
end
...
% file_data = cellfun(@(x) isequal(x,table2cell(BLinfo)),num2cell(table2cell(net_type),2));
% file_data = result_data(file_data,1);
% file_data = cat(1,file_data{:});
% 
% BL_in_leg = false;
% if ~isempty(file_data) %skip plot if no data...
...
%             file_data = log10(file_data);
%     end
%     histogram(file_data,'Normalization','probability','FaceColor',BLcol,'EdgeColor',BLcol,'FaceAlpha',alph);
%     BL_in_leg = true;BL_mu = mean(file_data);
% end
    ...
    otherwise
        ...
        if main_in_leg
            ...
            leg_labels = [leg_labels;sprintf('%.0f Hz %s (\\mu=%.1f)',...
                unique(job_params.trial_stimuli),job_params.stim_targs,main_mu)];
        end
%         if BL_in_leg
...
xlabel(Zlabel,'Fontsize',14,'FontWeight','bold')
...
num_states = numel(file_data);
fprintf('states found:\n')
fprintf('--- n = %i\n',num_states)
...
%     for j = 1:2
%         
%         file_data = table2cell(curr_net_info(j,IDvars));
%         file_data = cellfun(@(x) isequal(x,file_data),... %ugly indexing & transform here..
%             num2cell(table2cell(need_more(:,IDvars)),2));
%         if sum(file_data) > 0
%             file_data = find(file_data);
%             fprintf('\nnetwork #%i %s has < 10k states',idx,curr_net_info.targ_cells{j})
%             fprintf('\n---parameter sets:\n')
%             for h = 1:numel(file_data)
%                 disp(need_more(file_data(h),:))
%                 fprintf('--- n = %i\n',current_count(file_data(h)))
%             end
%             fprintf('\n------------------------\n')
%         end
%         
...
%         file_data = cellfun(@(x) isequal(x,table2cell(BLinfo)),... %ugly indexing & transform here..
...
%                 fprintf('---type: %s\n',curr_net_info{j,end})
%                 %find the right results for network set-up
%                 file_data = cellfun(@(x) isequal(x,curr_net_info(j,:)),net_type,'UniformOutput',false);
%                 file_data = cat(1,file_data{:});
%                 file_data = result_data{file_data,1};
%                 switch outcome_stat
%                     case 'logmu'
%                         file_data = log10(file_data(file_data~=0));
%                 end
%                 Xstim{j} = file_data;
%                 
%                 fprintf('            stimulus = %.2f\n',curr_net_info{j,3})
