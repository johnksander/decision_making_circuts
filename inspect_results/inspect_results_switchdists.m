...
config_options.sim_name = 'switching_bias3_10kSwitches';
...
%aggregate results
trial_currents = unique(options.trial_currents(:,1));
num_trials = numel(trial_currents);
...
for idx = 1:num_trials
    trials = trial_currents(idx) == options.trial_currents(:,1);
...
for idx = 1:num_trials
    trials = trial_currents(idx) == options.trial_currents(:,1);
    real_trial_currents(idx,1) = unique(options.trial_currents(trials,1));
    real_trial_currents(idx,2) = unique(options.trial_currents(trials,2));
end
