...
fig_dir = fullfile(fig_dir,'stability_test');
if ~isdir(fig_dir),mkdir(fig_dir);end
fig_name = 'combfig';

conv_txtdump = 'off'; %off/on
how2load = 'txtdump'; %'txtdump' | 'regular'
min_time = .75; %min switch time in seconds (for txtdump function)
...
switch how2load
    ...
    case 'txtdump'
        output_fn = [options.modeltype '_' options.sim_name '_stabilitytest_txtdump.mat'];
end
