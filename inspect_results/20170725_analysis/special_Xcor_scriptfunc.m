function  special_Xcor_scriptfunc(celltype,targIDs,Xcor_method)
...
output_dir = fullfile(output_dir,'preswitch_comparisons');
...
%select data
cell_data = cell(numel(celltype),num_models);
for select_idx = 1:numel(celltype)
    ID2get = [targIDs{1},celltype{select_idx}]; %just so func input below is a little easier...
...
for plot_idx = 1:num_models
    ...
    ylabel(sprintf(Yax_labs,celltype{1},celltype{2}))
    xlabel(sprintf(Xax_labs,celltype{2}))
...
grand_title([targIDs{1}{1} ' (' targIDs{1}{2} ')'],16)

%print it out
% fig_XcorID = [strrep(celltype{1},'-','') 'X' strrep(celltype{2},'-','')];
% fig_fn = strrep(char(celltype),'-','');
% fig_fn = [fig_fn '_' fig_XcorID];
fig_fn = [strrep(celltype{1},'-','') 'X' strrep(celltype{2},'-','')];
