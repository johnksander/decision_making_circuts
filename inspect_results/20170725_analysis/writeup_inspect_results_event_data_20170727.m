...
fig_dir = fullfile(home_dir,'Results/writeup_results');
...
load(fullfile(output_dir,'event_data'),'info','event_data','event_windows','adjusted_switch_onsets')
...
% simlabels = {'fast-switch (no stimulus)','slow-switch (no stimulus)',...
%     'fast-switch (with stimulus)','slow-switch (w/ stim.)        '};
...
for figidx = 1:num_events
    if figidx == 3
        ...
        simlabels = {'fast-switch (no stimulus)','slow-switch (no stimulus)',...
            'fast-switch (with stimulus)','slow-switch (with stimulus)'};
    end
    ...
    for simidx = 1:num_sims
        ...
        if simidx == 3 || simidx == 4
            xlabel('transition timecourse (ms)')
        end
    ...
    print(fullfile(fig_dir,[fig_fn ' _timecourse']),'-djpeg')
    close all
end
