...
% linkaxes(h,'x')
% axis tight
% print(fullfile(figdir,'_duration_dists'),'-djpeg')

%five seperate vertically stacked plots
...
for idx = 1:num_types
    ...
    curr_net_info = network_pair_info{idx};
    swap_names = cellfun(@(x) strrep(x,'fast','Excite Stay'),curr_net_info(:,4),'UniformOutput',false);
    swap_names = cellfun(@(x) strrep(x,'slow','Excite Leave'),swap_names,'UniformOutput',false);
    
    figure(idx)
    for j = 1:2
        
        plt_idx = plt_idx + 1;
        h(plt_idx) = subplot(2,1,j);
        hold on
        
        %get the right title/color
        if strcmpi(swap_names{j},'Excite Leave')
            ftitle = 'Repel to leave';
            lcol = matorange;
        elseif strcmpi(swap_names{j},'Excite Stay')
            ftitle = 'Entice to stay';
            lcol = matblue;
        end
        ...
        if ~isempty(curr_data) %skip plot if no data...
            switch outcome_stat
                case 'logmu'
                    curr_data = log(curr_data);
                    [kde,kde_i] = ksdensity(curr_data);
                    plot(kde_i,kde,'LineWidth',5,'Color',lcol);
                case 'mu'
                    histogram(curr_data)
            end
        end
        ...
        if ~isempty(curr_data) %skip plot if no data...
            switch outcome_stat
                case 'logmu'
                    curr_data = log(curr_data);
                    [kde,kde_i] = ksdensity(curr_data);
                    plot(kde_i,kde,'LineWidth',5,'Color',BLcol);
                case 'mu'
                    histogram(curr_data)
            end
        end
        hold off
        legend({sprintf('%.0fHz %s',curr_net_info{j,3},swap_names{j}),'no stimulus'},'FontWeight','b')
        legend('boxoff')
        if j == 2
            xlabel('mean sampling log(s)','FontWeight','b')
        end
        title(ftitle,'FontWeight','b')
        ylabel('p(x)','FontWeight','b')
...
for idx = 1:num_types
    figure(idx)
    %print(fullfile(figdir,sprintf('brownbag_duration_dists_%i',idx)),'-djpeg')
end
f_xlims = get(gca,'XLim'); %fix "ideal fig" to these xlims
...
curr_net_info = network_pair_info{1}; %take network #1 baselines
swap_names = cellfun(@(x) strrep(x,'fast','Excite Stay'),curr_net_info(:,4),'UniformOutput',false);
swap_names = cellfun(@(x) strrep(x,'slow','Excite Leave'),swap_names,'UniformOutput',false);

result_net_info =  network_pair_info{3}; %take network #3 excite-stay result
result_net_info = result_net_info(2,:);
ideal_data = cellfun(@(x) isequal(x,result_net_info),net_type,'UniformOutput',false);
ideal_data = cat(1,ideal_data{:});
ideal_data = cell2mat(result_data(ideal_data,1));
ideal_data = log(ideal_data);
...
for j = 1:2
    ...
    [kde,kde_i] = ksdensity(ideal_data);
    plot(kde_i,kde,'LineWidth',5,'Color',lcol);
    ...
    hold off
    legend({sprintf('%s',swap_names{j}),'no stimulus'},'FontWeight','b')
    legend('boxoff')
    if j == 2
        xlabel('mean sampling log(s)','FontWeight','b')
    end
...
print(fullfile(figdir,'brownbag_duration_dists_ideal'),'-djpeg')
